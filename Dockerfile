...
RUN apt upgrade -y
RUN apt install -y software-properties-common apt-transport-https curl sudo gnupg pipenv unzip mysql-client postgresql-client dnsutils wget git nodejs npm python3 python3-pip python3-tk
RUN wget -q https://download.oracle.com/java/24/latest/jdk-24_linux-x64_bin.deb
RUN dpkg -i ./jdk-24_linux-x64_bin.deb
RUN rm -f jdk-24_linux-x64_bin.deb
RUN update-alternatives --install /usr/bin/java java /usr/lib/jvm/jdk-24.0.1-oracle-x64/bin/java 2424
